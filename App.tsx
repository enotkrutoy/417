        ...
        {step === 'SELECT' && (
          <div className="max-w-4xl mx-auto space-y-12 py-10">
            <div className="text-center space-y-6">
              ...
              <p className="text-slate-400 text-lg max-w-xl mx-auto font-medium">Спецификация 2020: Версия 10. Точная генерация битового потока PDF417.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="group relative bg-slate-900/50 border border-white/5 rounded-[3rem] p-10 hover:border-sky-500/40 transition-all cursor-pointer shadow-2xl overflow-hidden active:scale-[0.98]" onClick={() => !isScanning && fileInputRef.current?.click()}>
                ...
                <p className="text-slate-400 text-sm leading-relaxed mb-10 font-medium italic">Автоматическая экстракция тегов через Gemini 3 Flash Node.</p>
            ...
            <div className="lg:col-span-8 bg-slate-900/60 rounded-[3.5rem] p-8 sm:p-12 border border-white/5 shadow-2xl space-y-10 backdrop-blur-xl relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                <div className="space-y-2.5">
                  <label className="text-[10px] font-black text-slate-500 uppercase flex justify-between tracking-widest px-1">ID / DL No. <span>DAQ</span></label>
                ...
                <div className="space-y-2.5">
                  <label className="text-[10px] font-black text-slate-500 uppercase flex justify-between tracking-widest px-1">Expiration <span>DBA</span></label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                 <div className="space-y-2.5 md:col-span-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase flex justify-between tracking-widest px-1">Audit / DD Node <span>DCF</span></label>
                 ...
                 <div className="space-y-2.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase flex justify-between tracking-widest px-1">Card Revision <span>DDB</span></label>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-sky-500/5 rounded-[2.5rem] p-8 border border-sky-500/10 space-y-4">
                <button onClick={() => setShowTechSpecs(!showTechSpecs)} className="w-full text-[10px] font-black text-sky-400 uppercase tracking-widest flex items-center justify-between group italic">
                  <span className="flex items-center gap-2 group-hover:text-white transition-colors"><Cpu size={16}/> Matrix Specs</span>
                ...
                {showTechSpecs && (
                  <div className="text-[10px] text-slate-400 space-y-3 animate-in slide-in-from-top-2 duration-300 font-mono">
                    <div className="p-4 bg-black/40 rounded-[1.5rem] border border-white/5 space-y-3 shadow-inner">
                       <p className="flex justify-between border-b border-white/5 pb-1"><span>HDR_IIN:</span> <span className="text-sky-500">{formData.IIN}</span></p>
                       <p className="flex justify-between border-b border-white/5 pb-1"><span>AAMVA_VER:</span> <span className="text-sky-500">10 (2020)</span></p>
                       <p className="flex justify-between border-b border-white/5 pb-1"><span>JUR_MTX:</span> <span className="text-sky-500">v.{formData.JurisdictionVersion}</span></p>
                       <p className="flex justify-between"><span>ECC_LVL:</span> <span className="text-emerald-500">5</span></p>
                    </div>
                    {validation.complianceNotes.length > 0 && (
                      <div className="p-4 bg-rose-500/10 rounded-[1.5rem] border border-rose-500/20 text-rose-300 space-y-2">
                        {validation.complianceNotes.map((note, idx) => <p key={idx} className="leading-snug italic">! {note}</p>)}
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-700">
            <div className="flex flex-col sm:flex-row justify-between items-end gap-6 border-b border-white/5 pb-8">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <span className="px-3 py-1 bg-sky-500/10 text-sky-400 rounded-lg text-[10px] font-black uppercase tracking-wider border border-sky-500/20 shadow-lg italic">Grade B Compliant</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-[4rem] p-12 text-slate-950 flex flex-col items-center gap-12 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border-4 border-slate-200 print:shadow-none print:border-none">
              <div className="text-center space-y-3">
                ...
                <div className="flex items-center justify-center gap-4">
                   <span className="bg-slate-100 px-4 py-1.5 rounded-full text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] font-mono italic">SYNT_KERNEL_v1.10</span>
                </div>
              </div>
              
              <div className="w-full transition-transform hover:scale-[1.01] duration-500">
              ...
              <button onClick={() => window.print()} className="w-full max-w-md bg-slate-950 text-white py-7 rounded-[2.5rem] font-black text-2xl hover:bg-slate-800 hover:-translate-y-1 transition-all shadow-[0_20px_50px_rgba(0,0,0,0.3)] active:translate-y-0 flex items-center justify-center gap-4 group italic">
                <Printer size={32} className="group-hover:scale-110 transition-transform"/> PRINT MASTER CORE
              </button>
            </div>

            <div className="bg-slate-900/50 border border-white/5 p-10 rounded-[3.5rem] space-y-6 backdrop-blur-md shadow-2xl">
               <div className="flex justify-between items-center">
                  <h4 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] flex items-center gap-3 italic"><Terminal size={16} /> Bitstream Output</h4>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-3xl flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-md rounded-[3rem] p-12 border border-white/10 shadow-[0_50px_100px_-20px_rgba(14,165,233,0.3)] space-y-10 animate-in zoom-in-95 duration-300">
            <div className="flex items-center gap-6">
               ...
               <div>
                  <h3 className="text-3xl font-black tracking-tight italic">Extraction Node</h3>
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mt-1 font-mono">Gateway Protocol</p>
               </div>
            </div>
            <div className="space-y-6">
               ...
               <p className="text-[10px] text-slate-500 font-medium px-2 italic text-center leading-relaxed">Данный ключ используется локально для инициализации контура OCR Vision.</p>
            </div>
            <div className="flex gap-4">
              ...
              <button onClick={() => { localStorage.setItem('gemini_api_key', apiKey); setIsSettingsOpen(false); }} className="flex-1 bg-sky-600 hover:bg-sky-500 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl active:scale-95 italic">Update Kernel</button>
            </div>
          </div>
        </div>
      )}
